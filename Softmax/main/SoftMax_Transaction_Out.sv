...
`include "uvm_pkg.sv"
import uvm_pkg::*;
class SoftMax_Transaction_Out#(int Matrix_Row=197,int Matrix_Col=197,int bitsize=64) extends uvm_sequence_item;

    bit[bitsize-1:0]Matrix[];



    string File_Loc="Softmax_Out.txt";
    `uvm_object_utils_begin(SoftMax_Transaction_Out);//field_automation机制
        `uvm_field_array_int(Matrix,UVM_ALL_ON);
    ...
    function bit Mycompare(SoftMax_Transaction_Out to_Compare,string str1="[Expect]",string str2="[Actual");
        bit result=1;
        $display("getting in compare now");
        $display("getting in compare now");
        $display("getting in compare now");
        for (int i=0;i<Matrix_Row*(Matrix_Col/8+1);i=i+1)begin
            if(Matrix[i]==to_Compare.Matrix[i])begin
                ...
                result=0;
                // $display("[%d] %s %h---%s %h",i,str1,Matrix[i],str2,to_Compare.Matrix[i]);
