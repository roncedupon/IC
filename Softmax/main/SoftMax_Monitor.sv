...
class Stage2_ComputeZP_Monitor extends uvm_monitor;
    virtual Axis vif;
    bit DUMP_FLAG=0;
    string Fil_Loc="Stage2_ComputeZP_Indata.txt";
    uvm_analysis_port#(SoftMax_Transaction_Out) ap;//需要将监视到的Transaction传递给Ref_Model,

    `uvm_component_utils(Stage2_ComputeZP_Monitor);
    function new(string name="Stage2_ComputeZP_Monitor",uvm_component parent=null);
        super.new(name,parent);
    endfunction
...
task Stage2_ComputeZP_Monitor::main_phase(uvm_phase phase);
    SoftMax_Transaction_Out tr;
    while(1)begin
        tr=new("tr");
        collect_one_pkg(tr);
...
task Stage2_ComputeZP_Monitor::collect_one_pkg(SoftMax_Transaction_Out tr);
    bit[63:0]data_q[$];
    int i=0;
    while(1)begin
        @(posedge vif.clk)
