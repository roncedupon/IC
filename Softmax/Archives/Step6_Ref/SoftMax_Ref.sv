...
class SoftMax_Ref extends uvm_component;
    ...
    int Matrix_Row;
    int Matrix_Col;
    extern function new(string name,uvm_component parent,int Matrix_Row=197,int Matrix_Col=197);
...
function SoftMax_Ref::new(string name,uvm_component parent,int Matrix_Row=197,int Matrix_Col=197);
        ...
        for(int i=0;i<Matrix_Row;i=i+1)begin//遍历行
            ...
            for(int j=0;j<Matrix_Col/8+1;j=j+1)begin
                for(int k=0;k<8;k=k+1)begin
                    if(Max<SoftMax_Input.Matrix[i][k*8+:8])
                        Max=SoftMax_Input.Matrix[i][k*8+:8];
                end
            end
            ...
            for(int j=0;j<Matrix_Col/8+1;j=j+1)begin
                for(int k=0;k<8;k=k+1)begin
                    SoftMax_Std_Output.Matrix[i][i*k+:8]=$signed(SoftMax_Input.Matrix[i][i*k+:8])-Max;
                end
            end
        end
