...
module Stage2_ComputeZP#(
    ...
    input logic clk,
    input logic rstn,
    input logic[15:0]Scale,//这个就是(-S/ln2)<<16取整后的值----这里放大2^16倍完全够用,如果位宽超了,那就放大2^15倍试试精度
...
generate;
    ...
    for(i=0;i<8;i=i+1)begin
        Mul#(.AWidth(8),.BWidth(16),.CWidth(24),.delay_times(Zdelay_times))Compute_Z(//计算Z
            .clk(clk),//conpute Z=floor(Xq*Scale)
            .rstn(rstn),
            .A(sData[i*8+:8]),
            .B(Scale),
            .C(Z_Tmp[i])
...
wire [63:0]Z_Tmp2;
for (i=0;i<8;i=i+1)begin
    assign Z_Tmp2[i*8+:8]=Z_Tmp[i][23:16];
end
...
generate;
    for(i=0;i<8;i=i+1)begin
        Mul#(.AWidth(8),.BWidth(8),.CWidth(16),.delay_times(Pdelay_times))Compute_P(
            ...
            .rstn(rstn),
            .A(Scale2),
            .B(Z_Tmp[i][23:16]),//取整后的Z
            .C(P_Tmp[i])//
        );
        wire [15:0]P_tmp2;
        assign P_tmp2=({sDAta_Dly[i*8+:8],{8'd0}}-P_Tmp[i]);//此处是不是需要加一个signed？
        assign P[i*P_WIDTH+:P_WIDTH]=P_tmp2[7:0];//此处是不是需要加一个signed？
    end
endgenerate
