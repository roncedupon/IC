...
logic [7:0]Last_Max;
logic Col_Cnt_Valid;
assign Col_Cnt_Valid =Col_Cnt==count_times-1&&sValid;
...
reg [7:0]Max_4[0:3];//对进来的8个点进行两两比较，取4个最大值
...
always_ff@(posedge clk or negedge rstn)begin
    if (!rstn)
        ...
        for (int i = 0; i <4; i = i+1)
            Max_4[i] <= sData[2*i*8+:8]<sData[(2*i+1)*8+:8]?sData[(2*i+1)*8+:8]:sData[2*i*8+:8];//>sData[7:0]?sData[7:0]:sData[7:0];//对比较结果打一拍
    end
end

reg [7:0]Max_2[0:1];//对上一级的4个点继续进行两两比较，取2个最大值
...
always_ff @(posedge clk or negedge rstn) begin
    ...
    else mValid<=IS_GET_ONE_COL&sValid&sReady;//进一个数，往外面蹦一个数
end
...
wire Row_Cnt_Valid;
assign Row_Cnt_Valid=(Row_Cnt==Matrix_Row)&&mValid;//Row_Cnt需要多数一行
always_ff@(posedge clk or negedge rstn)begin
    if(!rstn)Row_Cnt<=0;
