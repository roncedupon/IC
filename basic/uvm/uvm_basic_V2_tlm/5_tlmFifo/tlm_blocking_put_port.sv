...
class blocking_port_producer extends uvm_component;
    ...
    virtual task run_phase(uvm_phase phase);
        ...
        for(int i = 0; i < 10; i=i+1) begin
            ...
            #50
            put_port.put(t);
            $display("[%0d][producer put port]:producer put one tr to consumer",$time);
        end
...
class blocking_port_comsumer extends uvm_component;
    ...
    virtual task run_phase(uvm_phase phase);
        ...
        for(int i=0;i<10;i=1+1)begin
            #100
            get_port.get(t);
            $display("[%0d][consumer put export]:consumer get one tr from producer",$time);
        end
        phase.drop_objection(this);
    endtask
    ...
    virtual task run_phase(uvm_phase phase);
        forever begin
            ...
            if(fifo_producer2consumer.is_full())
                $display("[%0d]FIFO is now full !!",$time);
        end
    endtask
endclass
