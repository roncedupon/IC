...
class father extends uvm_component;
    ...
    virtual task run_phase(uvm_phase phase);
        son son_tmp0;//←←←现在代码没问题了←←←
        son son_tmp1;//←←←现在代码没问题了←←←
        ...
        // `define TEST
        //=========================================================================================================
        // son_tmp0=get_child("son0");//Illegal class assignment---比如像这样直接用=赋值是会报错的
        $cast(son_tmp0,get_child("son0"));
        $cast(son_tmp1,get_child("son1"));
        `uvm_info("get_child",$sformatf("son_tmp0.id is %0d",son_tmp0.id),UVM_LOW);
        `uvm_info("get_child",$sformatf("son_tmp1.id is %0d",son_tmp1.id),UVM_LOW);
        //=========================================================================================================
        // `uvm_info("get_child",$sformatf("son_tmp0.father is %s",get_child("son0").get_parent().get_full_name()),UVM_LOW);
        // `uvm_info("get_child",$sformatf("son_tmp1.father is %s",get_child("son1").get_parent().get_full_name()),UVM_LOW);
