...
`include "i2c_master_miss_rep_start_after_start_byte_seq.sv"
...
class i2c_master_miss_rep_start_after_start_byte_test extends i2c_base_test;

  i2c_master_miss_rep_start_after_start_byte_callback cust_callback;

  /** UVM component utility macro */
  `uvm_component_utils(i2c_master_miss_rep_start_after_start_byte_test)

  /** Class constructor */
  function new(string name = "i2c_master_miss_rep_start_after_start_byte_test", uvm_component parent=null);
  ...
  virtual function void build_phase(uvm_phase phase);
   ...
   uvm_config_db#(uvm_object_wrapper)::set(this, "env.i2c_system_env.master[0].sequencer.main_phase", "default_sequence", i2c_master_miss_rep_start_after_start_byte_seq::type_id::get());
  ...
  task main_phase(uvm_phase phase);
    ...
    `uvm_info("main_phase", $sformatf("Setting the drain time in the main_phase of the i2c_master_miss_rep_start_after_start_byte_test"), UVM_NONE) 
      `ifdef SVT_UVM_1800_2_2017_OR_HIGHER
  ...
  endtask : main_phase
   ...
   //Follwoing errors expected:
   //1. i2c_dsize_size_8_bit: Since no repeatd start arrives for start byte, frame is corrupted and this error will be expected.
   //2. i2c_invalid_data_change: Since no repeatd start arrives for start byte, frame is corrupted and this error will be expected.
   //3. i2c_repeat_start_to_follow_start_byte: Error for no rep start after start byte
   //4. i2c_invalid_reserved_slave_id: Because of no rep-start, data sampled incorrectly making this error flash.
   //5. i2c_ack_received_for_invalid_slave_id: Because of no rep-start, data sampled incorrectly making this error flash.
   function void disable_error();
    ...
    env.i2c_system_env.master[0].monitor.err_check.slave_shall_not_issue_address_ack.set_default_fail_effect(svt_err_check_stats::NOTE);
    ...
    env.i2c_system_env.slave[0].monitor.err_check.slave_shall_not_issue_address_ack.set_default_fail_effect(svt_err_check_stats::NOTE);
