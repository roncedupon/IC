 ...
 * A simple constrained random test is shown in this example. 
 * It disables the base test case virtual sequence on the virtual sequencer.
 * After that a random sequence is set on the agent's sequencers.
 */
...
class random_test extends i2c_base_test;
  ...
  virtual function void build_phase(uvm_phase phase);
   ...
   /** Apply the random i2c sequence to the i2c master sequencer */
    ...
    /** Apply the random i2c sequence to the i2c slave sequencer */
    ...
    uvm_config_db#(int unsigned)::set(this, "env.i2c_system_env.master[0].sequencer.i2c_mst_random_sequence", "sequence_length", 2);
    uvm_config_db#(int unsigned)::set(this, "env.i2c_system_env.slave[0].sequencer.i2c_slv_random_sequence", "sequence_length", 2);
  ...
  task main_phase(uvm_phase phase);
    ...
    `uvm_info("main_phase", $sformatf("Setting the drain time in the main_phase of the base \test to twice the configured IPG Length 200"), UVM_NONE) 
    `ifdef SVT_UVM_1800_2_2017_OR_HIGHER
      phase_d = phase.get_objection();
      phase_d.set_drain_time(this, (20000));
    `else
