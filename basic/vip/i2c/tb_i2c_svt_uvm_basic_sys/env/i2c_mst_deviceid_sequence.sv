...
`ifndef GUARD_I2C_MST_DEVICEID_SEQUENCE_UVM_SV
`define GUARD_I2C_MST_DEVICEID_SEQUENCE_UVM_SV
...
class i2c_mst_deviceid_sequence extends uvm_sequence #(svt_i2c_master_transaction); 
  ...
  `uvm_object_utils(i2c_mst_deviceid_sequence)
  ...
  /** Class constructor */
  function new (string name = "i2c_mst_deviceid_sequence");
    super.new(name);
  endfunction : new
...
`ifdef SVT_UVM_1800_2_2017_OR_HIGHER
    `uvm_rand_send(tx_xacts_m, -1,
		       {
		         tx_xacts_m.cmd             == I2C_DEVICE_ID; //I2C_WRITE ;
...
endclass : i2c_mst_deviceid_sequence 

`endif // GUARD_I2C_DEVICEID_SEQUENCE_UVM_SV
