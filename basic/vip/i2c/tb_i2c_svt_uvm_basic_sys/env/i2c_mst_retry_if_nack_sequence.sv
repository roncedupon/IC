...
`ifndef GUARD_I2C_MST_RETRY_IF_NACK_SEQUENCE_UVM_SV
`define GUARD_I2C_MST_RETRY_IF_NACK_SEQUENCE_UVM_SV
...
class i2c_mst_retry_if_nack_sequence extends uvm_sequence #(svt_i2c_master_transaction); 
  ...
  `uvm_object_utils(i2c_mst_retry_if_nack_sequence)
  ...
  /** Class constructor */
  function new (string name = "i2c_mst_retry_if_nack_sequence");
    super.new(name);
  endfunction : new
...
`uvm_rand_send(tx_xacts_m, -1,
		  {
		    ...
		    tx_xacts_m.retry_if_nack   == 1 ;
		    tx_xacts_m.num_of_retry    == 3 ;
		  })
`else
...
endclass : i2c_mst_retry_if_nack_sequence 

`endif // GUARD_I2C_MST_RETRY_IF_NACK_SEQUENCE_UVM_SV
