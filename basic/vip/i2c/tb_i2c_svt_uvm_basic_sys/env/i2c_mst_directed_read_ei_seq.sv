...
`ifndef GUARD_I2C_MST_DIRECTED_READ_EI_SEQ_UVM_SV
`define GUARD_I2C_MST_DIRECTED_READ_EI_SEQ_UVM_SV
...
class i2c_mst_directed_read_ei_seq extends uvm_sequence #(svt_i2c_master_transaction); 
  ...
  `uvm_object_utils(i2c_mst_directed_read_ei_seq)
  ...
  /** Class constructor */
  function new (string name = "i2c_mst_directed_read_ei_seq");
    super.new(name);
  endfunction : new
...
`endif
    ...
    if (!req.randomize() with
                { req.addr                 == `SVT_I2C_SLAVE0_ADDRESS;
                  ...
                  req.do_insert_error      == 1; //ERROR INJECTED
                })
                `uvm_error("Randomization failure"," i2c_mst_directed_read_ei_seq")
    `uvm_send(req)
...
endclass : i2c_mst_directed_read_ei_seq 
