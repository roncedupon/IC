`ifndef GUARD_I2C_MASTER_INSERT_STOP_AFTER_8BIT_OF_READ_DATA_BYTE_CALLBACK_SV
`define GUARD_I2C_MASTER_INSERT_STOP_AFTER_8BIT_OF_READ_DATA_BYTE_CALLBACK_SV

/**
 * Abstract: 
 * class 'i2c_master_insert_stop_after_8bit_of_read_data_byte_callback' is extended from svt_i2c_master_callback class. 
 * It implements the post_seq_item_get method to set error exception from master.
 * It is used to send illegal Stop inspite of ACK bit after 8th bit of read data byte.
 */

class i2c_master_insert_stop_after_8bit_of_read_data_byte_callback extends svt_i2c_master_callback;

  /**
   * This variable is used to tell the master after how many bytes it should send illegal stop.
   * Possible values
  ...
  virtual function void post_seq_item_get(svt_i2c_master driver, svt_i2c_master_transaction xact, ref bit drop);
    ...
    exception.error_kind = svt_i2c_master_transaction_exception::I2C_INSERT_P_AFTER_8_BIT_RD_DATA;
    if(num_data_byte == 0)
...
endclass: i2c_master_insert_stop_after_8bit_of_read_data_byte_callback
`endif //GUARD_I2C_MASTER_INSERT_STOP_AFTER_8BIT_OF_READ_DATA_BYTE_CALLBACK_SV
