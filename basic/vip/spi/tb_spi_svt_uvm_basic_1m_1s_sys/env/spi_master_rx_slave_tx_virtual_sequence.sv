...
`ifndef GUARD_SPI_MASTER_RX_SLAVE_TX_VIRTUAL_SEQUENCE_SV
`define GUARD_SPI_MASTER_RX_SLAVE_TX_VIRTUAL_SEQUENCE_SV
...
class spi_master_rx_slave_tx_virtual_sequence extends uvm_sequence #(uvm_sequence_item,uvm_sequence_item);

  /** Sequence Length in Virtual Sequence, to set to the actual default sequence */
  int unsigned sequence_length = 1;

  /** UVM object utility macro */
  `uvm_object_utils(spi_master_rx_slave_tx_virtual_sequence)
  ...
  /** Class constructor */
  function new (string name = "spi_master_rx_slave_tx_virtual_sequence");
    super.new(name);
  endfunction : new
...
`endif
  ...
  virtual task body();
    ...
    spi_rxonly_sequence mstr_seq;
    spi_txonly_sequence slave_seq;
...
endclass : spi_master_rx_slave_tx_virtual_sequence 

`endif //GUARD_SPI_MASTER_RX_SLAVE_TX_VIRTUAL_SEQUENCE_SV 
