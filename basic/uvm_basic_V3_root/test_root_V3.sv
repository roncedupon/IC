...
// `include "tlm_tr.sv"
import uvm_pkg::*;
//这个版本用来测试uvm_root在哪，但是失败了，因为run_test("uvm_top")或run_test("uvm_root")都fatal了
...
class son extends uvm_component;
    ...
    virtual function void build_phase(uvm_phase phase);
        ...
        $display("display root name:%s",uvm_root::get().get_full_name());//fail
        uvm_root::get().get_first_child(str_tmp);
        $display("first child of root:%s",str_tmp);//success
    endfunction
endclass
