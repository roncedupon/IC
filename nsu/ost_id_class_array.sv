...
// 2. Core mapping class: Support multiple map arrays (m_ost_trans_map_array[map_idx][ost_id])
class OstIdTransMap;
    // ===================== Embedded type definition (core optimization) =====================
    typedef transaction_t transaction_queue_t[$];

    // ===================== Member variables: Array of maps (NEW FEATURE) =====================
    // m_ost_trans_map_array[map_idx][ost_id] = transaction_queue_t
    // map_idx: Index of the map array (e.g., 0, 1, 2...)
    // ost_id:  Key of the associative array inside each map
    protected transaction_queue_t m_ost_trans_map_array[int][int];

    // ===================== Core method: Add single transaction (with map_idx) =====================
    function automatic void add_single_trans(int map_idx, int ost_id, transaction_t trans);
        if(trans == null) begin
            $error("[OstIdTransMap] Failed to add transaction: null handle!");
            return;
        end
        // Check if map_idx exists, if not, initialize it
        if(!m_ost_trans_map_array.exists(map_idx)) begin
            m_ost_trans_map_array[map_idx] = '{};
        end
        // Check if ost_id exists inside the map, if not, initialize it
    ...
    // ===================== Auxiliary method: Clear entire map (all ost_ids in a map) =====================
    function automatic void clear_entire_map(int map_idx);
        if(m_ost_trans_map_array.exists(map_idx)) begin
            // Clear all queues in the map first
    ...
    // ===================== Auxiliary method: Get all existing map indices =====================
    ...
    // ===================== Auxiliary method: Get all existing ost_ids in a specific map =====================
...
// 4. Testbench: Test multiple map arrays
module tb_ost_id_trans_map;
    OstIdTransMap                ost_map;
    transaction_t                trans1, trans2, trans3, trans4;
    OstIdTransMap::transaction_queue_t q1, q2, res_q;
    int                          map_indices[$], ids[$];
    ...
    initial begin
        ...
        $display("\n\n");
        $display("################################################################");
        $display("### TESTING MAP 0 ###");
        $display("################################################################");

        // Scenario 1: Add single transaction to Map 0
        trans1 = new(10, 32'h12345678, 4, 8'h01); 
        ost_map.add_single_trans(0, 10, trans1);

        // Scenario 2: Add batch transactions to Map 0
        ...
        $display("\n\n");
        $display("################################################################");
        $display("### TESTING MAP 1 ###");
        $display("################################################################");

        // Scenario 3: Add single transaction to Map 1
        trans4 = new(30, 32'hdeadbeef, 16, 8'hff); 
        ost_map.add_single_trans(1, 30, trans4);

        // Scenario 4: Add batch transactions to Map 1
        q2 = '{trans4, trans1}; // Reuse existing transactions
        ...
        $display("[Test] Does Map 1 exist: %0b", ost_map.has_map(1));
        $display("[Test] Does Map 2 exist: %0b", ost_map.has_map(2));

        // Check if ost_id exists in specific map
        $display("\n[Test] Does ost_id=10 exist in Map 0: %0b", ost_map.has_ost_id(0, 10));
        $display("[Test] Does ost_id=30 exist in Map 1: %0b", ost_map.has_ost_id(1, 30));
        $display("[Test] Does ost_id=30 exist in Map 0: %0b", ost_map.has_ost_id(0, 30));

        // Get queue from Map 0
        ...
        // Clear ost_id=20 in Map 0
        ost_map.clear_queue_by_ost_id(0, 20);
        $display("\n[Test] Queue length for ost_id=20 in Map 0 after clear: %0d", 
                 ost_map.get_queue_by_ost_id(0, 20).size());

        // Clear entire Map 1
        ost_map.clear_entire_map(1);
        $display("\n[Test] Does Map 1 exist after clear: %0b", ost_map.has_map(1));

        // Print final state
        $display("\n[Test] Print final state of all maps:");
        ost_map.print_all_maps();
